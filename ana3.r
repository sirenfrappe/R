...
dfP <- data.frame(matrix(1,leng+1,leng+1))
dfP[1,2:(leng+1)] <- sequ
dfP[2:(leng+1),1] <- sequ

df01 <- data.frame(matrix(1,leng,leng))
...
write.table(dfP,"D:\\idps\\script\\output\\rrfile\\P30443_dfP.txt",quote = FALSE,row.names = FALSE)
write.table(df01,"D:\\idps\\script\\output\\rrfile\\P30443_df01.txt",quote = FALSE,row.names = FALSE)
